...
[<Tests>]
let test = testList "Push tests" [
    testCase "It should" <| (fun _ ->
    let message = createMessage "title" "text" "hash"
    ["title";"text"] |> Seq.iter (fun i -> Assert.Equal("The property should exist and have the right value", i, message.Property(i).Value.ToString()))
